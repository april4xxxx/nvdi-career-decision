...
(function () {
  ...
  /* ---------- 初始地图任务（登基之初已在案的事务，避免地图空荡） ----------
     结构与生成任务一致：{title, cat, energy, gold, from} */
  var SEED_MAP_TASKS = [
    { title: "完成入职培训的结业答辩", cat: "main",    energy: 16, gold: 40, from: "入职清单" },
    { title: "整理并发出本周周报",     cat: "daily",   energy: 8,  gold: 15, from: "例行事务" },
    { title: "约同组前辈喝杯咖啡认识一下", cat: "explore", energy: 6, gold: 20, from: "融入团队" }
  ];
  ...
  var SCENARIOS = [
    {
      ...
      decision: {
        ...
        recommend: {
          ...
          tasks: [
            { title: "定选题 + 列一页大纲", cat: "main", energy: 6, gold: 15 },
            { title: "备稿并自己试讲一遍", cat: "main", energy: 12, gold: 25 }
          ]
        },
        alt: {
          label: "婉拒，把机会留到更有把握时",
          text: "如实说明近期项目吃紧，礼貌婉拒，并主动约定下一季度再来分享。",
          tasks: [{ title: "回复邀约并致谢、约定下次", cat: "daily", energy: 4, gold: 10 }]
        }
      }
    },
    {
      ...
      decision: {
        ...
        recommend: {
          ...
          tasks: [
            { title: "找出这门课的试听 / 退款政策", cat: "explore", energy: 4, gold: 10 },
            { title: "试学一节并写 3 行是否值得的判断", cat: "explore", energy: 8, gold: 20 }
          ]
        },
        alt: {
          label: "先用免费资源顶一个月",
          text: "先用官方文档、公开课把这块啃一个月。真卡住了再回来买，需求会清楚得多。",
          tasks: [{ title: "列一张免费替代资源清单", cat: "explore", energy: 5, gold: 12 }]
        }
      }
    },
    {
      ...
      decision: {
        ...
        recommend: {
          ...
          tasks: [{ title: "限时 20 分钟协助同事", cat: "daily", energy: 6, gold: 15 }]
        },
        alt: {
          label: "婉拒，给一个替代资源",
          text: "如实说自己正卡在 deadline 上，给他一个能自助的文档或人选，既不失礼也不揽责。",
          tasks: [{ title: "回复同事并附上替代资源", cat: "daily", energy: 3, gold: 8 }]
        }
      }
    },
    {
      ...
      decision: {
        ...
        recommend: {
          ...
          tasks: [{ title: "只做 15 分钟：启动第一步", cat: "delay", energy: 5, gold: 20 }]
        },
        alt: {
          label: "正式放弃并记录原因",
          text: "如果它其实没那么重要，就正式把它划掉，写一行为什么放弃。放下也是一种决策。",
          tasks: [{ title: "写一行放弃理由，正式了结", cat: "delay", energy: 3, gold: 10 }]
        }
      }
    },
    {
      ...
      decision: {
        ...
        recommend: {
          ...
          tasks: [{ title: "彻底休息 30 分钟（恢复精力）", cat: "mystic", energy: -30, gold: 10, restore: 30 }]
        },
        alt: {
          label: "只保当日底线，其余明日再说",
          text: "今天只交付一件必须交的，其余全部推到明天。允许自己有一个「低电量档」的一天。",
          tasks: [{ title: "只交付今日必做的一件事", cat: "main", energy: 8, gold: 15 }]
        }
      }
    }
  ];
  ...
  function genericDecision(text) {
    ...
    return {
      ...
      recommend: {
        ...
        tasks: [{ title: "完成「" + t + "」的第一小步", cat: "daily", energy: 6, gold: 15 }]
      },
      alt: {
        label: "再想清楚目标后重新规划",
        text: "先花几分钟写下你真正想要的结果是什么，再回来让臣为你重排优先级。",
        tasks: [{ title: "写清目标，稍后再议", cat: "delay", energy: 3, gold: 8 }]
      }
    };
  }
  ...
  function tasksFromPath(decision, pathKey) {
    ...
    return (path.tasks || []).map(function (t) {
      return {
        title: t.title,
        cat: t.cat || decision.category,
        energy: (typeof t.energy === "number") ? t.energy : 8,
        gold: (typeof t.gold === "number") ? t.gold : 15,
        restore: t.restore || 0,
        from: decision.title
      };
    });
  ...
  var ACHIEVEMENTS = [
    ...
    { cat: "青铜", idx: 2, id: "survey-six-ministries", name: "遍识六部", goal: "首次探访并认识全部六部(六个日常部门场景各进入一次)", hint: "走遍六部，认全每一处衙门", tier: 1, reward: "+30金", flavor: "知人知职，方能理事。", cur: 0, target: 6, unlocked: false, date: null, journalRefs: ["六部巡礼记"] },
    ...
    { cat: "青铜", idx: 4, id: "thirty-day-foothold", name: "立足登基", goal: "达成登基第30天并落印首月目标里程碑(完成阶段一主线)", hint: "撑到登基第30天，盖下首月里程碑印", tier: 2, reward: "称号·初立之主", flavor: "满月未缺，基石已定。", cur: 1, target: 30, unlocked: false, date: null, journalRefs: ["30天主线·熟悉阶段"] },
    { cat: "青铜", idx: 5, id: "first-solo-delivery", name: "独任其事", goal: "首次独立完成一项主线交付任务(无需辅助独立结项)", hint: "独立交付你的第一件差事", tier: 2, reward: "+50金", flavor: "不假人手，独当一面。", cur: 0, target: 1, unlocked: false, date: null, journalRefs: [] },
    ...
    { cat: "青铜", idx: 7, id: "sixty-day-reform", name: "整顿内政", goal: "达成登基第60天并落印阶段目标(完成阶段二主线)", hint: "行至登基第60天，内政井然，盖下阶段印", tier: 3, reward: "称号·中兴之主", flavor: "半程已过，宫阙焕然。", cur: 1, target: 60, unlocked: false, date: null, journalRefs: ["60天主线·成长阶段"] },
    ...
    { cat: "青铜", idx: 9, id: "ninety-day-coronation", name: "加冕正统", goal: "达成登基第90天并落印终章里程碑(通关三阶段全部主线)", hint: "走完九十日主线，举行加冕大典", tier: 5, reward: "称号·正统女帝", flavor: "九十日砺，今日加冕，名正而言顺。", cur: 1, target: 90, unlocked: false, date: null, journalRefs: ["90天主线·转正大典"] },
    // ---------- 瓷器·任务数(10) ----------
    ...
    { cat: "玉雕", idx: 4, id: "jade-low-energy-deliver", name: "韬光养晦", goal: "在精力低于20时仍完成一项任务的最小交付", hint: "精力低于20时，仍交付一项任务的底线成果", tier: 2, reward: "+30金", flavor: "光敛于内，力蓄于微。", cur: 0, target: 1, unlocked: false, date: null, journalRefs: [] },
    ...
    { cat: "玉雕", idx: 6, id: "jade-accumulate-500", name: "养精蓄锐", goal: "累计恢复精力达500点", hint: "累计恢复精力至500点", tier: 2, reward: "+40金", flavor: "锐气藏锋，待时而发。", cur: 0, target: 500, unlocked: false, date: null, journalRefs: [] },
    ...
    { cat: "玉雕", idx: 8, id: "jade-critical-complete", name: "气定神闲", goal: "在精力低于10时完成当日全部任务", hint: "精力告急低于10，仍从容做完当日任务", tier: 4, reward: "+60金", flavor: "气不乱则神自定。", cur: 0, target: 1, unlocked: false, date: null, journalRefs: [] },
    ...
    { cat: "玉雕", idx: 10, id: "jade-hold-cap-three-days", name: "元气淋漓", goal: "精力维持在上限150连续3日", hint: "连续三日把精力稳稳压在150上限", tier: 4, reward: "+80金", flavor: "元气充盈，举步生风。", cur: 0, target: 3, unlocked: false, date: null, journalRefs: [] },
    { cat: "玉雕", idx: 11, id: "jade-single-day-rebound", name: "返照回光", goal: "单日内精力从低于20回升至100以上", hint: "在同一日里，让精力从20以下反弹到100以上", tier: 2, reward: "+35金", flavor: "谷底一转，复见天光。", cur: 0, target: 1, unlocked: false, date: null, journalRefs: [] },
    { cat: "玉雕", idx: 12, id: "jade-accumulate-2000", name: "玉体康泰", goal: "累计恢复精力达2000点", hint: "累计恢复精力至2000点", tier: 3, reward: "称号·玉体康泰", flavor: "调养有方，体安神泰。", cur: 0, target: 2000, unlocked: false, date: null, journalRefs: [] },
    ...
    { cat: "玉雕", idx: 14, id: "jade-grand-harmony", name: "天人合一", goal: "累计恢复精力达5000点，且连续30日精力从未归零", hint: "累计恢复精力5000点，并连续三十日精力不归零", tier: 5, reward: "称号·天人合一", flavor: "内外相和，身与天齐。", cur: 0, target: 5000, unlocked: false, date: null, journalRefs: [] },
    // ---------- 金器·金钱(14) ----------
    ...
    { cat: "金器", idx: 7, id: "single-big-reward", name: "一掷千赏", goal: "单次任务或奏折获得100金及以上的高额奖励", hint: "单次奖励拿到100金以上，尝一回厚赏", tier: 3, reward: "+40金", flavor: "一赏倾金，足见其功。", cur: 0, target: 1, unlocked: false, date: null, journalRefs: [] },
    ...
    { cat: "金器", idx: 10, id: "first-spend", name: "量入为出", goal: "首次消费金币(如在钦天监恢复精力或购置道具)", hint: "第一次花掉金币，学会开源节流", tier: 1, reward: "+15金", flavor: "知取知舍，方善持家。", cur: 0, target: 1, unlocked: false, date: null, journalRefs: [] },
    { cat: "金器", idx: 11, id: "single-day-gold-200", name: "日纳百川", goal: "单日累计获得金币达到200", hint: "一天之内进账满200金", tier: 4, reward: "+80金", flavor: "一日之内，财如潮涌。", cur: 0, target: 200, unlocked: false, date: null, journalRefs: [] },
    ...
    { cat: "木器", idx: 5, id: "archive-first-read", name: "手不释卷", goal: "在藏书阁首次阅读一份档案或书籍", hint: "去藏书阁翻阅一份档案", tier: 1, reward: "+10金", flavor: "开卷有益，一读倾心。", cur: 0, target: 1, unlocked: false, date: null, journalRefs: ["治国之策·首阅"] },
    ...
    { cat: "木器", idx: 12, id: "pizhu-dadan", name: "破格取才", goal: '首次以"大胆"朱批批准一份奏折', hint: '用"大胆"朱批放手批准一次', tier: 3, reward: "+20金", flavor: "不拘一格，方见胆识。", cur: 0, target: 1, unlocked: false, date: null, journalRefs: [] }
  ];
